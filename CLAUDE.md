...
- `LLM_ENABLED` / `LLM_PROVIDER` / `LLM_API_KEY` / `LLM_BASE_URL` / `LLM_MODEL` — integração opcional de LLM (desativada por padrão), com suporte a múltiplos provedores via formato compatível com OpenAI: `anthropic`, `openai`, `gemini`, `openrouter`, `nvidia_build`, `cohere`. Só alimentam a linha inicial de `configuracoes_sistema` (banco anônimo) — o valor efetivo em runtime é sempre lido do banco, ajustável via `/admin/configuracoes`. O Chat de ajuda contextual (`/chat/mensagens`, `app/services/chat_ia.py`) é a primeira funcionalidade de IA de fato implementada; sugestão de questionário e análise de resultados continuam apenas como toggles reservados para o futuro.
