...
- `LLM_ENABLED` / `LLM_PROVIDER` / `LLM_API_KEY` / `LLM_BASE_URL` / `LLM_MODEL` — integração opcional de LLM (desativada por padrão), com suporte a múltiplos provedores via formato compatível com OpenAI: `anthropic`, `openai`, `gemini`, `openrouter`, `nvidia_build`, `cohere`. Só alimentam a linha inicial de `configuracoes_sistema` (banco anônimo) — o valor efetivo em runtime é sempre lido do banco, ajustável via `/admin/configuracoes`. Os três recursos de IA (chat de ajuda contextual `app/services/chat_ia.py`, criação assistida de questionário `app/services/questionario_ia.py`, análise assistida de resultados `app/services/resultados_ia.py`) estão implementados, cada um com toggle próprio, canalizados por `app/services/llm_client.py` (ponto único de chamada ao provedor).
- `CORS_ORIGINS` — origens permitidas pelo flask-cors (separadas por vírgula), padrão `http://localhost:5173`.
- `SESSAO_LOGIN_TTL_HORAS` — validade do token de sessão em horas (padrão 24).
